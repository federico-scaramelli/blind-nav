...
namespace CMF
{
	...
	public class AdvancedWalkerController : Controller {
		...
		protected CharacterMovementInput characterMovementInput;
		...
		protected virtual Vector3 CalculateMovementDirection()
		{
			...
			{
				_velocity += tr.right * characterMovementInput.GetHorizontalMovementInput();
				_velocity += tr.forward * characterMovementInput.GetVerticalMovementInput();
			}
			else
			{
				//If a camera transform has been assigned, use the assigned transform's axes for movement direction;
				//Project movement direction so movement stays parallel to the ground;
				_velocity += Vector3.ProjectOnPlane(cameraTransform.right, tr.up).normalized * characterMovementInput.GetHorizontalMovementInput();
				_velocity += Vector3.ProjectOnPlane(cameraTransform.forward, tr.up).normalized * characterMovementInput.GetVerticalMovementInput();
			}
